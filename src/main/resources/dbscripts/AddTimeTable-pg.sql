...
CREATE VIEW durchgangeraet(wettkampf_id, durchgang, geraet, geraetname)
    ...
    from kutu.riege rrr
        inner join kutu.disziplin d on (d.id = start)
    where exists (select 1 from kutu.wertung w where rrr.name in (w.riege, w.riege2));
...
CREATE VIEW zeitplan(wettkampf_id, durchgang, einturnen, geraet, total)
    ...
    from (
        ...
        from (
            ...
            inner join kutu.riege rr on (rr.wettkampf_id = w.wettkampf_id and rr.name in (w.riege, w.riege2) and d.id in (
                select geraet from kutu.durchgangeraet dg where dg.wettkampf_id = w.wettkampf_id and dg.durchgang = rr.durchgang
            ))
            inner join kutu.wettkampf_plan_zeiten pzz on (pzz.wettkampf_id = w.wettkampf_id and pzz.wettkampfdisziplin_id = w.wettkampfdisziplin_id)
