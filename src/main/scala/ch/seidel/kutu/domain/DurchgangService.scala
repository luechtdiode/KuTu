...
trait DurchgangService extends DBService with DurchgangResultMapper {
  ...
  def selectDurchgangstationen(wettkampfId: Long): List[Durchgangstation] = {
  ...
  def selectDurchgangstationenView(wettkampfId: Long): List[DurchgangstationView] = {
    Await.result(database.run{
  ...
  def renameDurchgangGroup(wettkampfid: Long, oldname: String, newname: String): Unit = {
    Await.result(database.run{
      sqlu"""
                update durchgang
                set title=${newname.trim}
  ...
  def selectDurchgaenge(wettkampfUUID: UUID): Seq[Durchgang] = {
    Await.result(selectDurchgaengeAsync(wettkampfUUID), Duration.Inf)
  }

  private def selectDurchgaengeAsync(wettkampfUUID: UUID) = {
    database.run{
      sql"""select
  ...
  def selectSimpleDurchgaenge(wettkampfId: Long): Seq[SimpleDurchgang] = {
    Await.result(selectSimpleDurchgaengeAsync(wettkampfId), Duration.Inf)
  }

  private def selectSimpleDurchgaengeAsync(wettkampfId: Long) = {
    database.run{
      sql"""select
               d.id, d.wettkampf_id, d.title, d.name, d.durchgangtype, d.ordinal,
               d.planStartOffset, d.effectiveStartTime, d.effectiveEndTime
