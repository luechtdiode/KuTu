...
abstract trait WertungService extends DBService with WertungResultMapper with DisziplinService with RiegenService with JsonSupport {
  ...
  def updateOrinsertWertungenZuWettkampf(wettkampf: Wettkampf, wertungen: Iterable[Wertung]) = {
    val insertWertungenAction = DBIO.sequence(for {
      ...
      sqlu"""
                insert into wertung
                (athlet_Id, wettkampfdisziplin_Id, wettkampf_Id, note_d, note_e, endnote, riege, riege2, team, mediafile, variables)
                values (${w.athletId}, ${w.wettkampfdisziplinId}, ${w.wettkampfId}, ${w.noteD}, ${w.noteE}, ${w.endnote}, ${w.riege}, ${w.riege2}, ${w.team.getOrElse(0)}, ${w.mediafile}, ${w.variables.map(_.toJson.compactPrint)})
        """
    })
  ...
  def updateAllWertungenAsync(ws: Seq[Wertung]): Future[Seq[WertungView]] = {
    ...
    val ret = database.run(DBIO.sequence(for {
      ...
      sqlu"""       UPDATE wertung
                    SET note_d=${w.noteD}, note_e=${w.noteE}, endnote=${w.endnote}, riege=${w.riege}, riege2=${w.riege2}, team=${w.team.getOrElse(0)}, mediafile=${w.mediafile}, variables=${w.variables.map(_.toJson.compactPrint)}
                    WHERE id=${w.id}
          """>>
      sqlu"""       DELETE from riege
                    WHERE wettkampf_id=${w.id} and not exists (
  ...
  def validateWertung(w: Wertung, cache2:scala.collection.mutable.Map[Long, List[ScoreCalcTemplate]] = scala.collection.mutable.Map[Long, List[ScoreCalcTemplate]]()): Wertung = {
    ...
    try {
      ...
      case e:IllegalArgumentException =>
        throw new IllegalArgumentException(s"Endwert kann nicht berechnet werden: ${e.getMessage}")
    }
  }
  ...
  def updateWertungSimple(w: Wertung, cache2: scala.collection.mutable.Map[Long, List[ScoreCalcTemplate]] = scala.collection.mutable.Map[Long, List[ScoreCalcTemplate]]()): Wertung = {
    ...
    Await.result(database.run(DBIO.sequence(Seq(sqlu"""
                  UPDATE wertung
                  SET note_d=${wv.noteD}, note_e=${wv.noteE}, endnote=${wv.endnote}, riege=${wv.riege}, riege2=${wv.riege2}, team=${wv.team.getOrElse(0)}, mediafile=${w.mediafile}, variables=${w.variables.map(_.toJson.compactPrint)}
                  WHERE 
  ...
  def updateWertungenWithIDMapping(ws: Seq[Wertung], cache2: scala.collection.mutable.Map[Long, List[ScoreCalcTemplate]] = scala.collection.mutable.Map[Long, List[ScoreCalcTemplate]]()): Seq[Wertung] = {
    ...
    val wvId: Seq[Option[Long]] = Await.result(database.run(DBIO.sequence(for {
      ...
      sqlu"""
                  UPDATE wertung
                  SET note_d=${wv.noteD}, note_e=${wv.noteE}, endnote=${wv.endnote}, riege=${wv.riege}, riege2=${wv.riege2}, team=${wv.team.getOrElse(0)}, mediafile=${wv.mediafile}, variables=${wv.variables.map(_.toJson.compactPrint)}
                  WHERE
                    athlet_Id=${wv.athletId} and wettkampfdisziplin_Id=${wv.wettkampfdisziplinId} and wettkampf_Id=${wv.wettkampfId}
          """>>
      sql"""
