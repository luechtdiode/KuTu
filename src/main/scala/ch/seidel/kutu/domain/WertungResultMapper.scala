...
trait WertungResultMapper extends WettkampfResultMapper with DisziplinResultMapper with AthletResultMapper {
    ...
    WertungView(r.<<[Long], r, r, r, r.<<[Option[scala.math.BigDecimal]], r.<<[Option[scala.math.BigDecimal]], r.<<[Option[scala.math.BigDecimal]], r.<<, r.<<))
    ...
    implicit def getResultWertung = GetResult(r => Wertung(r.<<, r.<<, r.<<, r.<<, r.<<, r.<<, r.<<, r.<<, r.<<?, r.<<?))
