...
import ch.seidel.kutu.renderer.{CompetitionsClubsToHtmlRenderer, CompetitionsJudgeToHtmlRenderer}
                ...
                if userId.equals(competitionId.toString) then {
                  put {
                    entity(as[List[AthletView]]) { athletlist =>
                      ...
                      val hadRiege2 = hasWettkampfRiege2Assignments(wettkampf.id)
                      sexchanges.foreach(aa => adjustWertungRiegen(wettkampf, this, aa._1.toAthlet, hadRiege2))
                            ...
                            val officialCandidates = selectAthletesView(Verein(reg.vereinId.getOrElse(0), reg.vereinname, Some(reg.verband)))
                              ...
                              .map {
                                case a@AthletView(id, _, geschlecht, name, vorname, gebdat, _, _, _, _, _) =>
                                  AthletRegistration(0L, reg.id, Some(id), geschlecht, name, vorname, gebdat.map(dateToExportedStr).getOrElse(""), 0L, 0, Some(a), None, None, 0)
                              }
                                  ...
                                  if metadata.contentType.mediaType.fileExtensions.contains(fileext) then {
                                    ...
                                    val processor = Future {
                                      ...
                                      if id > 0 then {
                                        ...
                                        AthletRegistration(id, 0, None, "", "", "", "", 0L, 0L, None, None, Some(ResourceExchanger.saveMediaFile(is, wettkampf, Media("", metadata.fileName, fileext))), 0)
                                      }
                                    }
