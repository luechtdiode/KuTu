  ...
  lazy val wettkampfRoutes: Route = {
    handleCID { (clientId: String) =>
      ...
      pathPrefixLabeled("competition" / JavaUUID, "competition/:competition-id") { wkuuid =>
        ...
        pathLabeled("judge-link", "judge-link") {
          get {
            authenticatedAdmin() { userId =>
              if userId.equals(wkuuid.toString) then {
                complete {
                  Future {
                    val claims = setClaims(wkuuid.toString, jwtTokenExpiryPeriodInDays, isAdmin = false)
                    val shortJwt = JsonWebToken(jwtHeader, claims, jwtSecretKey)
                    val enc = java.util.Base64.getUrlEncoder
                    val link = s"$remoteBaseUrl/?" + new String(enc.encodeToString(s"c=$wkuuid&s=$shortJwt".getBytes))
                    val qrImage = ch.seidel.kutu.renderer.ServerPrintUtil.toQRCodeImage(link)
                    JudgeLink(link, qrImage)
                  }
                }
              } else {
                complete(StatusCodes.Conflict)
              ...
              if userId.equals(wkuuid.toString) then {
                onSuccess(readWettkampfAsync(wkuuid.toString)) { wk =>
                  ...
                  if athletwertungen.nonEmpty then {
                    unassignAthletFromWettkampf(athletwertungen, publishWS = false)
                  }
            ...
            authenticatedAdmin() { userId =>
              if userId.equals(wkuuid.toString) then {
                get {
                  complete {
                    CompetitionCoordinatorClientActor.publish(
                      GetRiegenEinteilungState(wkuuid.toString), clientId
                    ).mapTo[RiegenEinteilungStateUpdated].map(_.state.riegen)
                      .map(items => items.toJson(using listFormat(using riegeItemFormat)))
                  }
                } ~
                put {
              ...
              authenticatedAdmin() { userId =>
                if userId.equals(wkuuid.toString) then {
                  complete {
                    CompetitionCoordinatorClientActor.publish(
                      GetRiegenEinteilungState(wkuuid.toString), clientId
                    ).mapTo[RiegenEinteilungStateUpdated].map(_.state.duration)
                      .map(items => items.toJson(using listFormat(using durchgangDurationItemFormat)))
                  }
                } else {
                  complete(StatusCodes.Conflict)
                }
