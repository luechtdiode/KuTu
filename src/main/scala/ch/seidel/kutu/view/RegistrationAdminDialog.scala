          ...
          cellValueFactory = { x =>
            new ReadOnlyStringWrapper(x.value, "verein", {
              x.value match {
                ...
                case RenameAthletAction(verein, _, _) => s"${verein.vereinname} (${verein.verband})"
          ...
          cellValueFactory = { x =>
            new ReadOnlyStringWrapper(x.value, "athlet", {
              x.value match {
                ...
                case RenameAthletAction(_, existing, _) => s"${existing.extendedprint}"
          ...
          cellValueFactory = { x =>
            new ReadOnlyStringWrapper(x.value, "dbmatch", {
              x.value match {
                ...
                case RenameAthletAction(verein, _, expected) => s"Athlet wird auf ${expected.extendedprint} korrigiert"
                case AddRegistration(reg, programId, athlet, suggestion) =>
          ...
          cellValueFactory = { x =>
            new ReadOnlyStringWrapper(x.value, "assignaction", {
              x.value match {
                ...
                case RenameAthletAction(verein, _, _) => ""
      ...
      text.addListener { (o: ObservableValue[_ <: String], oldVal: String, newVal: String) =>
        ...
        for {syncAction <- athletModel } {
          val (athlet, verein) = syncAction match {
            ...
            case RenameAthletAction(verein, existing, expected) => (expected, Some(verein.toVerein))
          }
