...
class RanglisteTab(wettkampf: WettkampfView, override val service: KutuService) extends DefaultRanglisteTab(service) {
  ...
  override def groupers: List[FilterBy] = {
    List(ByNothing, ByWettkampfArt, ByWettkampfProgramm(programmText), ByProgramm(programmText), ByJahrgang, ByGeschlecht, ByVerband, ByVerein, ByRiege, ByDisziplin)
  }
  ...
  override def isPopulated = {
    ...
    combos(0).selectionModel.value.select(ByWettkampfProgramm(programmText))
