...
class WettkampfSpec extends KuTuBaseSpec {
  "wettkampf" should {
    "create with disziplin-plan-times" in {
      val wettkampf = createWettkampf(LocalDate.now(), "titel", Set(20), 33, 0, None)
    ...
    "update" in {
      val wettkampf = createWettkampf(LocalDate.now(), "titel2", Set(20), 33, 0, None)
      val wettkampfsaved = saveWettkampf(wettkampf.id, wettkampf.datum, "neuer titel", Set(wettkampf.programmId), 10000, 7.5, wettkampf.uuid)
    ...
    "recreate with disziplin-plan-times" in {
      ...
      val views = initWettkampfDisziplinTimes(UUID.fromString(wettkampf.uuid.get))
      assert(views.nonEmpty)
      val wettkampf2 = createWettkampf(LocalDate.now(), "titel2", Set(20), 33, 0, wettkampf.uuid)
    ...
    "delete all disziplin-plan-time entries when wk is deleted" in {
      val wettkampf = createWettkampf(LocalDate.now(), "titel3", Set(20), 33, 0, None)
