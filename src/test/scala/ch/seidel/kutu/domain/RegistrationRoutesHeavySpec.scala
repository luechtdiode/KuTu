...
class RegistrationRoutesHeavySpec extends KuTuBaseSpec {
  ...
  "registration routes heavy endpoint coverage" should {

    "update verein via sync-admin endpoint and reject registration token" in {
      val wk = insertGeTuWettkampf("HeavyUpdateVerein", 0)
      val reg = newRegistration(wk, "HeavyVereinClub")
    ...
    "return InternalServerError for sync-admin athletes bulk update on SQLite" in {
      val wk = insertGeTuWettkampf("HeavyUpdateAthletes", 0)
    ...
    "copy athletes and judges from another competition without duplicates" in {
      val sourceWk = insertGeTuWettkampf("HeavyCopySource", 0)
      val targetWk = insertGeTuWettkampf("HeavyCopyTarget", 0)
    ...
    "return conflict on mediafile get/delete when no media is linked" in {
      val wk = insertGeTuWettkampf("HeavyMediaNoFile", 0)
    ...
    "reject mediafile upload when content type is not mp3" in {
      val wk = insertGeTuWettkampf("HeavyMediaInvalidType", 0)
