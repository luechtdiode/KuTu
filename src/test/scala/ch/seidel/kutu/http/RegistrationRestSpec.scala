...
class RegistrationRestSpec extends KuTuBaseSpec {
  ...
  "registration" should {
    ...
    "authentication and account lifecycle scenarios" should {
      ...
      "protect by login with registration id" in {
        val reg = createTestRegistration
      ...
      "enable password-reset after failed login with correct registration ID" in {
        val reg = createTestRegistration
        Mailbox.clearAll()

        // Step 1: login attempt with correct registration ID but wrong password
        // → 401 Unauthorized + a JWT whose user-id ends with OPTION_LOGINRESET
        val unsuccessfulLogin = HttpRequest(method = POST, uri = "/api/login", entity = "")
          .addHeader(Authorization(BasicHttpCredentials(reg.id.toString, "wrong password")))
        ...
        unsuccessfulLogin ~> allroutes(x => vereinSecretHashLookup(x), id => extractRegistrationId(id)) ~> check {
          ...
          val loginresetJwt = header(Config.jwtAuthorizationKey).get
          val host         = "test-origin.ch:5678"
          val encodedOrigin = s"https://$host"
          // Referer must contain "/registration" so the server can derive the origin
          val referer = s"$encodedOrigin/registration/${testwettkampf.uuid.get}/${reg.id}"

          // Step 2: use the loginreset JWT to trigger the password-reset email
          HttpRequest(method = GET,
            uri = s"/api/registrations/${testwettkampf.uuid.get}/${reg.id}/loginreset")
            .addHeader(RawHeader("Host", host))
            .addHeader(RawHeader("Referer", referer))
            .addHeader(loginresetJwt) ~>
            allroutes(x => vereinSecretHashLookup(x), id => extractRegistrationId(id)) ~> check {
            status should ===(StatusCodes.OK)
          }
        }
      ...
      "reject password-reset when a regular (non-loginreset) JWT is used" in {
        createTestRegistration
        val path = s"/api/registrations/${testwettkampf.uuid.get}/${registration.get.id}/loginreset"

        // A regular registration JWT does NOT carry the OPTION_LOGINRESET suffix
        // → the route must respond with Conflict
      ...
      "reject password-reset when the loginreset token targets a different registration" in {
        val reg = createTestRegistration
        ...
        unsuccessfulLogin ~> allroutes(x => vereinSecretHashLookup(x), id => extractRegistrationId(id)) ~> check {
          ...
          // The loginreset token is bound to reg.id; requesting reset for id 99999999 → Forbidden
          HttpRequest(method = GET,
            uri = s"/api/registrations/${testwettkampf.uuid.get}/99999999/loginreset")
            .addHeader(loginresetJwt) ~>
            allroutes(x => vereinSecretHashLookup(x), id => extractRegistrationId(id)) ~> check {
            status should ===(StatusCodes.Forbidden)
          }
    ...
    "judge registration scenarios" should {
      ...
      "athlete registration scenarios" should {
      ...
      "registration authorization and deletion scenarios" should {
